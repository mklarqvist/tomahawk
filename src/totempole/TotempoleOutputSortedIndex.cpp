...
bool TotempoleOutputSortedIndex::findOverlap(const U32 contigID, const U32 position, totempole_entry& intervals){
	...
	const U32 chunk = position / ((this->contigs[contigID].n_bases >> TOTEMPOLE_OUTPUT_SORT_SHIFT) + 1);
...
bool TotempoleOutputSortedIndex::findOverlap(const U32 contigID, const U32 from, const U32 to, std::vector<totempole_entry>& intervals){
	...
	const U32 chunkFrom = from / ((this->contigs[contigID].n_bases >> TOTEMPOLE_OUTPUT_SORT_SHIFT) + 1);
	U32 chunkTo         = to   / ((this->contigs[contigID].n_bases >> TOTEMPOLE_OUTPUT_SORT_SHIFT) + 1);
