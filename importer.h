...
public:
	...
	bool Import(void){
		...
		outstream->open("/media/mdrk/NVMe/1kgp3/debug.twk",std::ios::out | std::ios::binary);
		if(!outstream->good()){
		...
		//*//////////////// Repoen and compute
		twk_reader reader;
		if(reader.Open("/media/mdrk/NVMe/1kgp3/debug.twk") == false){
		...
		LDEngine engine;
		engine.SetSamples(vcf->vcf_header_.GetNumberSamples());
		...
		int method = 0;
			...
			for(int b1 = 0; b1 < index.n; ++b1){
				...
				for(int b2 = b1+1; b2 < index.n; ++b2){
					...
					if(fl_lim++ == 10){
						std::cerr << "m=" << method << " " << utility::ToPrettyString(n_total) << " " << utility::ToPrettyString((uint64_t)((float)n_total/timer.Elapsed().count())) << "/s " << utility::ToPrettyString((uint64_t)((float)n_total*vcf->vcf_header_.GetNumberSamples()/timer.Elapsed().count())) << "/gt/s " << timer.ElapsedString() << " " << utility::ToPrettyString(engine.t_out) << std::endl;
						fl_lim = 0;
					}
				}
				out_of_range:
