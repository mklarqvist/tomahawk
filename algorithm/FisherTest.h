...
public:
	FisherTest(const U32 number) : number(number), logN_values(this->number, 0){
		this->Build();
	}
	~FisherTest(){}
	...
	double chiSquaredTest(T& a, T& b, T& c, T& d) const{
		...
		if(chisq < 0){
			//std::cerr << "negative chisq!!!" << std::endl;
			//std::cerr << adjustValue << '\t' << a<< '\t' << b << '\t' << c << '\t' << d << std::endl;
	...
	inline double kf_gammap(double s, double z){return z <= 1. || z < s? _kf_gammap(s, z) : 1. - _kf_gammaq(s, z);}
	inline double kf_gammaq(double s, double z){return z <= 1. || z < s? 1. - _kf_gammap(s, z) : _kf_gammaq(s, z);}
